...
def count_video_files_and_size(input_dir,video_file_extension):
    ...
    total_size = 0
    unique_recordings = 0
    videos_raw_folders = 0
    
    #check if there are any videos-raw folders in the directory
    #this check ensures that video files and videos-raw folders
    #are captured correctly
    folder_check = (
        os.path.basename(os.path.normpath(input_dir)) == 'videos-raw'
        or any(
            d == 'videos-raw' and os.path.isdir(os.path.join(root, d))
    ...
    return total_files, total_size, unique_recordings, videos_raw_folders, error_string